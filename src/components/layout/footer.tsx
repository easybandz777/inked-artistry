            ...
            <div className="mx-auto max-w-7xl px-6 lg:px-8 pt-16 pb-10">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10 lg:gap-8">
                    ...
                    <div className="lg:col-span-2">
                        ...
                        <p className="mt-4 text-sm text-warm-white/50 leading-relaxed max-w-sm">
                            {SITE_CONFIG.description}
                        </p>
                        <div className="mt-6 space-y-3">
                            <a href={`tel:${SITE_CONFIG.phone}`} className="flex items-center gap-3 text-sm text-warm-white/60 hover:text-rose-gold transition-colors">
                                <Phone className="h-4 w-4" />
                                {SITE_CONFIG.phone}
                            </a>
                            <a href={`mailto:${SITE_CONFIG.email}`} className="flex items-center gap-3 text-sm text-warm-white/60 hover:text-rose-gold transition-colors">
                    ...
                    <div>
                        ...
                        <ul className="space-y-3">
                            {FOOTER_LINKS.services.map((link) => (
                                <li key={link.href}>
                                    <Link href={link.href} className="text-sm text-warm-white/50 hover:text-rose-gold transition-colors">
                                        {link.label}
                                    </Link>
                                </li>
                            ))}
